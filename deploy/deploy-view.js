...
export default function PMDeployView({itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption, 
	closeModal, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.PM_DEPLOY_PAGE != null) {
    	columns = itemState.prefLabels.PM_DEPLOY_PAGE;
    }
    let group = "TABLE1";
    
    let header = "";
	if (itemState.prefTexts.PM_DEPLOY_PAGE != null && itemState.prefTexts.PM_DEPLOY_PAGE.PM_DEPLOY_PAGE_HEADER != null) {
		header = itemState.prefTexts.PM_DEPLOY_PAGE.PM_DEPLOY_PAGE_HEADER.value;
	}
