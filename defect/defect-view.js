...
export default function PMDefectView({itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption, 
	closeModal, inputChange, goBack, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.PM_DEFECT_PAGE != null) {
    	columns = itemState.prefLabels.PM_DEFECT_PAGE;
    }
    let group = "TABLE1";
	...
	if (itemState.prefTexts.PM_DEFECT_PAGE != null && itemState.prefTexts.PM_DEFECT_PAGE.PM_DEFECT_PAGE_HEADER != null) {
		header = itemState.prefTexts.PM_DEFECT_PAGE.PM_DEFECT_PAGE_HEADER.value;
	}
